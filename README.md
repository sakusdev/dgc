...
| `DISCORD_ALLOWED_CHANNEL_ID` | `/ask` の使用を許可するDiscordテキストチャンネルID |
...
`DISCORD_ALLOWED_CHANNEL_ID` の確認方法:
...
この値が未設定の場合、Botは安全のためすべてのチャンネルで利用を拒否します。異なるチャンネルで `/ask` を使うと、許可されたチャンネルへの案内だけが本人に表示されます。
...
```json
{"ok":true,"service":"dgc","model":"gemini-3.1-pro-preview","channelRestrictionConfigured":true}
```
