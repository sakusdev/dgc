  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          maxOutputTokens: clampInteger(Number(env.MAX_OUTPUT_TOKENS || 2048), 256, 8192),
          temperature: 0.7,
        },
      }),
...
function discordMessage(content: string, ephemeral: boolean): Response {
  return Response.json({ type: 4, data: { content, flags: ephemeral ? 64 : 0, allowed_mentions: { parse: [] } } });
}
...
async function editOriginalResponse(job: GeminiJob, content: string): Promise<void> {
  const response = await fetch(`${DISCORD_API}/webhooks/${job.applicationId}/${job.interactionToken}/messages/@original`, {
    method: "PATCH", headers: JSON_HEADERS, body: JSON.stringify({ content, allowed_mentions: { parse: [] } }),
  });
...
async function createFollowup(job: GeminiJob, content: string): Promise<void> {
  const response = await fetch(`${DISCORD_API}/webhooks/${job.applicationId}/${job.interactionToken}`, {
    method: "POST", headers: JSON_HEADERS, body: JSON.stringify({ content, allowed_mentions: { parse: [] } }),
  });
